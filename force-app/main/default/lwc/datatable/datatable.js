...
export default class Datatable extends LightningElement {
  ...
  get rowActionColumnOffset() {
    // v4.3.6 - for selecting correct filter column when action is on the left
  ...
  get hasRowAction() {
    // v4.3.5 isRemoveRowAction (as hasRowAction) is now used to track if any type of row action was configured
    return this._isRemoveRowAction;
  }

  get rowActionLeftOrRight() {
    // v4.3.5 removeRowLeftOrRight (as rowActionLeftOrRight) is now used to track whick column the action shows in
  ...
  set tableData(data = []) {
    if (this.isUpdateTable) {
      if (Array.isArray(data)) {
        ...
        console.log("this.columnFields ==" + JSON.stringify(this.columnFields));
  ...
  @api
  get not_suppressNameFieldLink() {
    // Default value is to show the links
  ...
  set serializedRecordData(value) {
    if (this.isSerializedRecordData && this.isUpdateTable) {
      ...
      this.dispatchEvent(
        new FlowAttributeChangeEvent("outputEditedRows", this.outputEditedRows)
      );
      this.dispatchEvent(
        new FlowAttributeChangeEvent(
          "numberOfRowsEdited",
          this.outputEditedRows.length
        )
      );
      this.outputEditedSerializedRows = "";
      this.dispatchEvent(
        new FlowAttributeChangeEvent(
          "outputEditedSerializedRows",
          this.outputEditedSerializedRows
        )
      );
      setTimeout(
  ...
  get isEmptyUserDefinedObject() {
    return (
      ...
      (!this._tableDataString || this._tableDataString?.length == 0)
    );
  }
  ...
  get pageTotalCount() {
    return Math.ceil(
      Number(Math.min(this.recordCountTotal, this.filteredRecordCount)) /
        Number(this._recordCountPerPage)
    );
  }
  ...
  handlePagination() {
    if (this.isPagination) {
      let firstRecord =
        (this._pageCurrentNumber - 1) * this._recordCountPerPage;
      let lastRecord = Math.min(
        this._pageCurrentNumber * this._recordCountPerPage,
        this.recordCountTotal
      );
      this.paginatedData = this.mydata.slice(firstRecord, lastRecord);
      let sids = [];
      this.allSelectedRowIds.forEach((srowid) => {
        const selRow = this._paginatedData.find(
          (d) => d[this.keyField] === srowid
        );
        sids.push(srowid);
      });
  ...
  get formattedTableLabel() {
    let filteredCount =
      this.filteredRecordCount != this.tableRecordCount
        ? `${this.filteredRecordCount} of `
        : "";
    let selectedCount = this.showSelectedCount
      ? ` • ${this.numberOfRowsSelected} selected`
      : "";
    return this.showRecordCount
      ? `${this._tableLabel} (${filteredCount}${this.tableRecordCount}${selectedCount})`
      : this._tableLabel;
  }
    ...
    if (data) {
      ...
      console.log(
        this.consoleLogPrefix +
          "getPicklistValuesByRecordType wire service returned error: " +
  ...
  connectedCallback() {
    ...
    console.log(
      "%cDATATABLE VERSION_NUMBER: %c" + CONSTANTS.VERSION_NUMBER,
      logStyleText,
      logStyleNumber
    );
    ...
    if (this.isConfigMode) {
      ...
      console.log(
        "from dataTable.js",
        this.consoleLogPrefix + "Config Mode Input columnAlignments:",
        this.columnAlignments
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnEdits:",
        this.columnEdits
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnFilters:",
        this.columnFilters
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnIcons:",
        this.columnIcons
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnLabels:",
        this.columnLabels
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnWidths:",
        this.columnWidths
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnWraps:",
        this.columnWraps
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnFlexes:",
        this.columnFlexes
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnFields:",
        this.columnFields
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnCellAttribs:",
        this.columnCellAttribs
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnTypeAttribs:",
        this.columnTypeAttribs
      );
      console.log(
        this.consoleLogPrefix + "Config Mode Input columnOtherAttribs:",
        this.columnOtherAttribs
      );
      this.not_suppressNameFieldLink = false;
    }

    console.log(
      this.consoleLogPrefix + "tableDataString - ",
      SHOW_DEBUG_INFO ? this._tableDataString : "***",
      this.isUserDefinedObject
    );
    ...
    console.log(
      this.consoleLogPrefix + "this._tableData",
      SHOW_DEBUG_INFO ? this._tableData : "***"
    );
    ...
    this.columnArray =
      this.columnFields.length > 0
        ? this.columnFields.replace(/\s/g, "").split(",")
        : [];
    this.columnFieldParameter = this.columnArray.join(", ");
    console.log(
      "from the datatable.js",
      this.consoleLogPrefix + "columnArray - ",
      this.columnArray
    );
    ...
    const parseAlignments =
      this.columnAlignments.length > 0
        ? this.columnAlignments.replace(/\s/g, "").split(",")
        : [];
    this.attribCount =
      parseAlignments.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
    ...
    if (this.columnEdits.toLowerCase() != "all") {
      const parseEdits =
        this.columnEdits.length > 0
          ? this.columnEdits.replace(/\s/g, "").split(",")
          : [];
      this.attribCount =
        parseEdits.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
    ...
    if (this.columnFilters.toLowerCase() != "all") {
      const parseFilters =
        this.columnFilters.length > 0
          ? this.columnFilters.replace(/\s/g, "").split(",")
          : [];
      this.attribCount =
        parseFilters.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
      this.filterAttribType = "none";
      parseFilters.forEach((filter) => {
        let colFilter =
          columnValue(filter).toLowerCase() == "true" ? true : false;
        ...
        this.filters.push({
          ...
          actions: [
            {
              label: this.label.SetFilterAction,
              disabled: false,
              name: "filter_" + col,
              iconName: "utility:filter"
            },
            {
              label: this.label.ClearFilterAction,
              disabled: true,
              name: "clear_" + col,
    ...
    const parseIcons =
      this.columnIcons.length > 0
        ? this.columnIcons.replace(/\s/g, "").split(",")
        : [];
    this.attribCount =
      parseIcons.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
    ...
    const parseLabels =
      this.columnLabels.length > 0
        ? removeSpaces(this.columnLabels).split(",")
        : [];
    this.attribCount =
      parseLabels.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
    ...
    if (this.isUserDefinedObject) {
      ...
      const parseScales =
        this.columnScales.length > 0
          ? removeSpaces(this.columnScales).split(",")
          : [];
      this.attribCount =
        parseScales.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
      parseScales.forEach((scale) => {
        this.scales.push({
          column: this.columnReference(scale),
          scale: columnValue(scale)
        });
        this.basicColumns[this.columnReference(scale)].scale =
          columnValue(scale);
      });
      ...
      const parseTypes =
        this.columnTypes.length > 0
          ? removeSpaces(this.columnTypes).split(",")
          : [];
      this.attribCount =
        parseTypes.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
    ...
    const parseWidths =
      this.columnWidths.length > 0
        ? this.columnWidths.replace(/\s/g, "").split(",")
        : [];
    this.attribCount =
      parseWidths.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
    ...
    if (this.columnFlexes.toLowerCase() != "all") {
      const parseFlexes =
        this.columnFlexes.length > 0
          ? this.columnFlexes.replace(/\s/g, "").split(",")
          : [];
      this.attribCount =
        parseFlexes.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
    ...
    const parseWraps =
      this.columnWraps.length > 0
        ? this.columnWraps.replace(/\s/g, "").split(",")
        : [];
    this.attribCount =
      parseWraps.findIndex((f) => f.search(":") != -1) != -1 ? 0 : 1;
    ...
    const parseCellAttribs =
      this.columnCellAttribs.length > 0
        ? removeSpaces(this.columnCellAttribs).split(";")
        : [];
    ...
    const parseOtherAttribs =
      this.columnOtherAttribs.length > 0
        ? removeSpaces(this.columnOtherAttribs).split(";")
        : [];
    this.attribCount = 0; // These attributes must specify a column number or field API name
    ...
    const parseTypeAttribs =
      this.columnTypeAttribs.length > 0
        ? removeSpaces(this.columnTypeAttribs).split(";")
        : [];
    this.attribCount = 0; // These attributes must specify a column number or field API name
    ...
    console.log(
      this.consoleLogPrefix + "tableHeightAttribute",
      this.tableHeightAttribute
    );
  ...
  assignApexDefinedRecords() {
    ...
    console.log(
      this.consoleLogPrefix + "tableData - ",
      SHOW_DEBUG_INFO ? this._tableData : "***"
    );
    this.preSelectedRows =
      this.preSelectedRowsString.length > 0
        ? JSON.parse(this.preSelectedRowsString)
        : [];
  }
  ...
  processDatatable() {
    if (this.isUserDefinedObject && !this.isConfigMode) {
      ...
      if (
        this.cols[this.rowActionColumnOffset]?.fieldName.endsWith("_lookup")
      ) {
      ...
      let fieldList =
        this.columnFields.length > 0
          ? this.columnFields.replace(/\s/g, "")
          : ""; // Remove spaces
      console.log(
        this.consoleLogPrefix + "Passing data to Apex Controller",
        SHOW_DEBUG_INFO ? data : "***"
      );
      getReturnResults({
        records: data,
        fieldNames: fieldList,
        suppressCurrencyConversion: this.suppressCurrencyConversion
      })
        .then((result) => {
          ...
          this.percentFieldArray =
            returnResults.percentFieldList.length > 0
              ...
              : [];
          this.numberFieldArray =
            returnResults.numberFieldList.length > 0
              ? returnResults.numberFieldList.toString().split(",")
              : [];
          this.timeFieldArray =
            returnResults.timeFieldList.length > 0
              ? returnResults.timeFieldList.toString().split(",")
              : [];
          this.datetimeFieldArray =
            returnResults.datetimeFieldList.length > 0
              ? returnResults.datetimeFieldList.toString().split(",")
              : [];
          console.log(
            this.consoleLogPrefix +
              "Datetime Fields ~ returnResults.datetimeFieldList.toString()",
            returnResults.datetimeFieldList.toString()
          );
          this.picklistFieldArray =
            returnResults.picklistFieldList.length > 0
              ? returnResults.picklistFieldList.toString().split(",")
              : [];
          ...
          console.log(
            this.consoleLogPrefix +
              "Picklist Fields ~ this.apex_picklistFieldMap",
            this.apex_picklistFieldMap
          );
          this.dateFieldArray =
            returnResults.dateFieldList.length > 0
              ? returnResults.dateFieldList.toString().split(",")
              : [];
          this.objectNameLookup = returnResults.objectName;
          this.objectLinkField = returnResults.objectLinkField;
          this.lookupFieldArray = JSON.parse(
            "[" + returnResults.lookupFieldData + "]"
          );
          this.timezoneOffset = returnResults.timezoneOffset.replace(
            /[^\d-]/g,
            ""
          ); // Numeric characters and - only
          console.log(
            this.consoleLogPrefix + "Timezone Offset ~ this.timezoneOffset",
            this.timezoneOffset
          );
          ...
          if (this.picklistReplaceValues) {
            ...
            this.picklistFieldArray.forEach((picklist) => {
              Object.keys(this.apex_picklistFieldMap[picklist]).forEach(
                (map) => {
          ...
          this.dtableColumnFieldDescriptorString =
            "[" + returnResults.dtableColumnFieldDescriptorString + "]";
          this.basicColumns = JSON.parse(
            this.dtableColumnFieldDescriptorString
          );
          console.log(
            this.consoleLogPrefix + "dtableColumnFieldDescriptorString",
            this.dtableColumnFieldDescriptorString,
            this.basicColumns
          );
          this.noEditFieldArray =
            returnResults.noEditFieldList.length > 0
              ? returnResults.noEditFieldList.toString().split(",")
              : [];
  ...
  updateDataRows() {
    ...
    let data = this.recordData
      ? JSON.parse(
          JSON.stringify([...this.recordData].slice(0, this.collectionSize))
        )
      : [];
    ...
    data.forEach((record) => {
      ...
      dateFields.forEach((date) => {
        if (record[date]) {
          ...
          if (!isNaN(dt)) {
            // Dates from External Objects are already formatted as Datetime (PR#1529)
            let d = new Date();
            let doffset = new Date(
              d.setTime(Number(dt) - Number(this.timezoneOffset))
            );
      ...
      lookupFields.forEach((lookup) => {
        if (this.isUserDefinedObject) {
          lufield = lookup;
          if (record[lufield]) {
            record[lufield + "_lookup"] =
              MYDOMAIN + record[lufield + "_lookup"];
          }
        } else {
          ...
          lookupFieldObject = this.lookupFieldArray.filter((obj) =>
            Object.keys(obj).some((key) => obj[key].includes(lufield))
          )[0];
          if (record[lufield]) {
            record[lufield + "_name"] =
              record[lufield][lookupFieldObject["nameField"]];
            record[lufield + "_id"] = record[lufield]["Id"];
            // Add new column with correct Lookup urls
            if (ISCOMMUNITY) {
              record[lufield + "_lookup"] =
                MYDOMAIN + "detail/" + record[lufield + "_id"];
            } else if (ISFLOWBUILDER) {
              record[lufield + "_lookup"] =
                MYDOMAIN + "/" + record[lufield + "_id"];
            } else {
              record[lufield + "_lookup"] =
                MYDOMAIN +
                ".lightning.force.com/lightning/r/" +
                lookupFieldObject["object"] +
                "/" +
                record[lufield + "_id"] +
                "/view";
            }
      ...
      if (!this.isUserDefinedObject || this.isConfigMode) {
        ...
        if (ISCOMMUNITY) {
          record[this.objectLinkField + "_lookup"] =
            MYDOMAIN + "detail/" + record["Id"];
        } else if (ISFLOWBUILDER) {
          record[this.objectLinkField + "_lookup"] =
            MYDOMAIN + "/" + record["Id"];
        } else {
        ...
        picklistFields.forEach((picklist) => {
          if (record[picklist]) {
            ...
            record[picklist].split(";").forEach((picklistValue) => {
              picklistLabels.push(
                this.apex_picklistFieldMap[picklist][picklistValue]
              );
            });
    ...
    console.log(
      this.consoleLogPrefix + "allSelectedRowIds",
      SHOW_DEBUG_INFO ? this.allSelectedRowIds : "***"
    );
    console.log(
      this.consoleLogPrefix + "keyField:",
      SHOW_DEBUG_INFO ? this.keyField : "***"
    );
    console.log(
      this.consoleLogPrefix + "tableData",
      SHOW_DEBUG_INFO ? this._tableData : "***"
    ...
    console.log(
      this.consoleLogPrefix + "mydata:",
      SHOW_DEBUG_INFO ? this._mydata : "***"
    );
    console.log(
      this.consoleLogPrefix + "outputRemainingRows:",
      SHOW_DEBUG_INFO ? this._outputRemainingRows : "***"
    );
  }
  ...
  updateColumns() {
    ...
    if (
      !this.isConfigMode &&
      this.hasRowAction &&
      this.rowActionLeftOrRight == "Left"
    )
      this.addRowAction();
    ...
    this.basicColumns.forEach((colDef) => {
      ...
      let alignmentAttrib = this.alignments.find(
        (i) => i["column"] == columnNumber
      );
      if (alignmentAttrib) {
      ...
      if (editAttrib) {
        switch (type) {
          ...
          case "time":
            ...
            break;
          default:
            if (this.noEditFieldArray.indexOf(fieldName) != -1)
              editAttrib.edit = false;
        }
        if (!editAttrib.edit) {
          this.isAllEdit = false;
      ...
      switch (this.flexAttribType) {
        case "cols":
          flexAttrib.flex =
            this.flexes.find((i) => i["column"] == columnNumber)?.flex || false;
          break;
        case "all":
      ...
      if (this.isConfigMode) {
        let wizardAlignLeft = !alignmentAttrib
          ? convertType(type) != "number"
          : alignment == "left";
        let wizardAlignCenter = !alignmentAttrib
          ? false
          : alignment == "center";
        let wizardAlignRight = !alignmentAttrib
          ? convertType(type) == "number"
          : alignment == "right";
        ...
        filterAttrib.actions = [
          {
            label: "Align Left",
            checked: wizardAlignLeft,
            name: "alignl_" + columnNumber,
            iconName: "utility:left_align_text"
          },
          {
            label: "Align Center",
            checked: wizardAlignCenter,
            name: "alignc_" + columnNumber,
            iconName: "utility:center_align_text"
          },
          {
            label: "Align Right",
            checked: wizardAlignRight,
            name: "alignr_" + columnNumber,
            iconName: "utility:right_align_text"
          },
          {
            label: "Select Icon",
            disabled: false,
            name: "icon_" + columnNumber,
            iconName: "utility:text"
          },
          {
            label: "Change Label",
            disabled: false,
            name: "label_" + columnNumber,
            iconName: "utility:text"
          },
          {
            label: "Cancel Change",
          ...
          },
          {
            label: "Allow Edit",
            checked: wizardEdit,
            name: "aedit_" + columnNumber,
            iconName: "utility:edit"
          },
          {
            label: "Allow Filter",
            checked: wizardFilter,
            name: "afilter_" + columnNumber,
            iconName: "utility:filter"
          },
          {
            label: "Flex Width",
            checked: wizardFlex,
            name: "flex_" + columnNumber,
            iconName: "utility:full_width_view"
          }
        ];
        this.cellAttributes = { alignment: alignment };
      }
      ...
      if (fieldName == this.objectLinkField && this.not_suppressNameFieldLink) {
        ...
        this.typeAttributes = {
          label: { fieldName: this.objectLinkField },
          target: this.linkTarget
        };
        if (editAttrib) {
      ...
      if (
        this.typeAttrib.type == "date-local" &&
        Object.keys(this.typeAttributes).length > 0
      ) {
        // If the user wants to override the default attributes, switch back to date (also switches to UTC time)
      ...
      this.cols.push({
        ...
        editable: editAttrib ? editAttrib.edit : false,
        actions:
          filterAttrib.filter && !this.hideHeaderActions
            ? filterAttrib.actions
            : null,
  ...
  addRowAction() {
    ...
    switch (this.rowActionDisplay) {
      ...
      case "Button":
        actionColumnWidth +=
          addCharacterCountWidth * this.rowActionButtonLabel.length;
        actionTypeAttributes = {
          name: actionType,
  ...
  updatePreSelectedRows() {
    ...
    if (!this.outputSelectedRows || this.outputSelectedRows.length === 0) {
      this.outputSelectedRows = this.preSelectedRows.slice(
        0,
      ...
      const selected = JSON.parse(
        JSON.stringify([...this.preSelectedRows.slice(0, this.maxNumberOfRows)])
      );
      let selectedKeys = [];
      ...
      this.preSelectedRows = [];
      this.dispatchEvent(
        new FlowAttributeChangeEvent("preSelectedRows", this.preSelectedRows)
      );
    }
  }
  ...
  columnReference(attrib) {
    ...
    if (this.attribCount == 0) {
      ...
      if (colDescriptor.endsWith("_lookup")) {
        colDescriptor = colDescriptor.slice(
          0,
          colDescriptor.lastIndexOf("_lookup")
        );
      }
        ...
        switch (propertyType) {
          ...
          case "type":
            ...
            if (result["name"] == "maximumFractionDigits") {
              ...
              let min = this.typeAttributes.minimumFractionDigits
                ? this.typeAttributes.minimumFractionDigits
                : 0;
              if (min > max) {
  ...
  handleRowAction(event) {
    ...
    console.log(
      this.consoleLogPrefix + "handleRowAction ~ action, keyValue:",
      action,
      SHOW_DEBUG_INFO ? keyValue : "***"
    );
    ...
    switch (action) {
      ...
      case "removeRow":
        if (
          this.maxRemovedRows == 0 ||
          this.numberOfRowsRemoved < this.maxRemovedRows
        ) {
          ...
          this.savePreEditData = [
            ...removeRowFromCollection(this, this._savePreEditData, keyValue)
          ];
          this.outputEditedRows = [
            ...removeRowFromCollection(this, this.outputEditedRows, keyValue)
          ];
          this.outputRemainingRows = [
            ...removeRowFromCollection(
              this,
          ...
          if (this.mydata.length == 0) {
            // Last record was removed from the datatable
            ...
            if (!this.isUserDefinedObject) {
              ...
              this.outputSelectedRowsString = JSON.stringify(
                this.outputSelectedRows
              );
              this.dispatchEvent(
                new FlowAttributeChangeEvent(
  ...
  handleComboValueChange(event) {
    ...
    let foundIndex = draftValues.findIndex(
      (value) => value[this.keyField] == eventDraftValue[this.keyField]
    );
  ...
  handleSave(event) {
    ...
    data = data.map((item) => {
      const draft = draftValues.find(
        (d) => d[this.keyField] == item[this.keyField]
      );
      if (draft != undefined) {
    ...
    edata = edata.map((eitem) => {
      const edraft = draftValues.find(
        (d) => d[this.keyField] == eitem[this.keyField]
      );
      if (edraft != undefined) {
        ...
        const orecord = this.outputEditedRows.find(
          (o) => o[this.keyField] == eitem[this.keyField]
        ); // Check to see if already in output collection
        if (orecord) {
          const otherEditedRows = [];
          this.outputEditedRows.forEach((er) => {
            // Remove current row so it can be replaced with the new edits
        ...
        datefield.forEach((date) => {
          if (!this.suppressBottomBar || date == editField) {
            try {
              if (field[date] && field[date].slice(-1) != "Z") {
                //Don't process if date has been converted to datetime because of TypeAttributes (v4.0.6)
                let rdt = Date.parse(field[date] + "T12:00:00.000Z"); //Set to Noon to avoid DST issues with the offset (v4.0.4));
                let rd = new Date();
                field[date] = new Date(
                  rd.setTime(Number(rdt) - Number(this.timezoneOffset))
                );
                field[date] = field[date].toISOString().slice(0, 10); // Winter '23 Patch 12 fix
              }
            } catch (err) {
              console.log(
                this.consoleLogPrefix + "Date not in ISO format",
                date,
                field[date]
              );
            }
        ...
        const isRemovedBeforeSave = this.outputRemovedRows.some(
          (rr) => rr[this.keyField] === eitem[this.keyField]
        );
        if (!isRemovedBeforeSave) {
          this.outputEditedRows = [...this.outputEditedRows, eitem]; // Add to output attribute collection
        }

        this.outputRemainingRows = replaceRowInCollection(
          this,
          this._outputRemainingRows,
          this.outputEditedRows,
          eitem[this.keyField]
    ...
    sdata = sdata.map((sitem) => {
      const sdraft = edata.find(
        (d) => d[this.keyField] == sitem[this.keyField]
      );
      if (sdraft != undefined) {
  ...
  handleRowSelection(event) {
    ...
    currentSelectedRows.forEach((selrow) => {
      const prevsel = this._allSelectedRowIds.some(
        (id) => id === selrow[this.keyField]
      );
      if (!prevsel) {
        this.allSelectedRowIds = [
          ...this._allSelectedRowIds,
          selrow[this.keyField]
        ];
      }
    });
    this._allSelectedRowIds.forEach((srowid) => {
      ...
      if (!found) {
        if (findRowIndexById(this, this.outputRemovedRows, srowid) == -1) {
          ...
          if (index != -1) {
            // Equals -1 if a previously selected row is no longer part of the current table data (reactivity)
        ...
        const stillSelected =
          findRowIndexById(this, currentSelectedRows, srowid) != -1;
        if (stillSelected) {
    ...
    this.outputSelectedRows = [];
    if (allSelectedRecs) {
      // Keep selected rows in the same order as the original table
      this._savePreEditData.forEach((rec) => {
        // Check all records - mydata would just be the filtered records here
        const isSelected = allSelectedRecs.some(
          (srec) =>
            srec[this.keyField] === rec[this.keyField] &&
            srec[this.keyField] !== undefined &&
            rec[this.keyField] !== undefined
        ); // PR1575 ignore when keyfield is not a valid field
        if (isSelected) {
  ...
  updateNumberOfRowsSelected(currentSelectedRows) {
    ...
    this.numberOfRowsSelected = this.singleRowSelection
      ? Math.min(1, currentSelectedRows.length)
      : currentSelectedRows.length;
    this.dispatchEvent(
      new FlowAttributeChangeEvent(
        "numberOfRowsSelected",
        this.numberOfRowsSelected
      )
    );
    ...
    this.outputSelectedRow =
      this.numberOfRowsSelected == 1 ? currentSelectedRows[0] : null;
    this.dispatchEvent(
      new FlowAttributeChangeEvent("outputSelectedRow", this.outputSelectedRow)
    );
    if (
      this.numberOfRowsSelected == 1 &&
      (this._tableData.length == 1 || this.singleRowSelection)
    ) {
      this.selectedRowKeyValue = this.outputSelectedRow[this.keyField]
        ? this.outputSelectedRow[this.keyField]
        : "";
      this.dispatchEvent(
        new FlowAttributeChangeEvent(
          "selectedRowKeyValue",
      ...
      this.showClearButton =
        !this.hideCheckboxColumn && !this.hideClearSelectionButton;
    }
  }
  ...
  handleClearFilterButton() {
    ...
    for (
      colNumber = this.rowActionColumnOffset;
      colNumber < colCount;
      colNumber++
    ) {
      let actionColRef = colNumber - this.rowActionColumnOffset; // *** v4.3.6 fix ***
      this.columns[colNumber].actions.find(
        (a) => a.name == "clear_" + actionColRef
      ).disabled = true;
      this.columns[colNumber].label =
        this.columns[colNumber].label.split(" [")[0];
    }
  ...
  updateColumnSorting(event) {
    ...
    this.dispatchEvent(
      new FlowAttributeChangeEvent("sortDirection", this.sortDirection)
    );
    this.doSort(this.sortedBy, this.sortDirection);
  }
  ...
  doSort(sortField, sortDirection) {
    ...
    if (sortField.endsWith("_lookup")) {
      sortField =
        sortField.slice(0, sortField.lastIndexOf("_lookup")) + "_name";
    }
      ...
      setTimeout(() => {
        if (this.isCaseInsensitiveSort && isString) {
          this.mydata = [
            ...this._mydata.sort(
              (a, b) => (
                (a = fieldValue(a).toLowerCase()),
                (b = fieldValue(b).toLowerCase()),
                reverse * ((a > b) - (b > a))
              )
            )
          ];
  ...
  handleHeaderAction(event) {
    ...
    if (
      !this.isConfigMode &&
      actionName.substr(actionName.length - 4) == "Text"
    ) {
      // Exit if system header action of wrapText or clipText
      return;
    }
    ...
    this.columnNumber =
      Number(colDef.actions[0].name.split("_")[1]) + this.rowActionColumnOffset; // v4.3.6 Use correct column when a left side row action is present
    this.baseLabel = this.filterColumns[this.columnNumber].label.split(" [")[0];
    const prompt = this.isConfigMode
      ? this.label.LabelHeader
      : this.label.FilterHeader;
    this.inputLabel =
      this.label.ColumnHeader + " " + prompt + ": " + this.baseLabel;
    switch (actionName.split("_")[0]) {
      case "alignl": // Config Mode Only
        this.filterColumns[this.columnNumber].cellAttributes = {
          alignment: "left"
        };
        this.filterColumns[this.columnNumber].typeAttributes["alignment"] =
          "slds-text-align_left"; //required for custom data table not using standard layout
        this.filterColumns[this.columnNumber].actions.find(
          (a) => a.name == "alignl_" + this.columnNumber
        ).checked = true;
        this.filterColumns[this.columnNumber].actions.find(
          (a) => a.name == "alignc_" + this.columnNumber
        ).checked = false;
        this.filterColumns[this.columnNumber].actions.find(
          (a) => a.name == "alignr_" + this.columnNumber
        ).checked = false;
      ...
      case "alignc": // Config Mode Only
        ...
        this.filterColumns[this.columnNumber].typeAttributes["alignment"] =
          "slds-text-align_center"; //required for custom data table not using standard layout
        this.filterColumns[this.columnNumber].actions.find(
          (a) => a.name == "alignl_" + this.columnNumber
        ).checked = false;
        this.filterColumns[this.columnNumber].actions.find(
      ...
      case "alignr": // Config Mode Only
        ...
        this.filterColumns[this.columnNumber].typeAttributes["alignment"] =
          "slds-text-align_right"; //required for custom data table not using standard layout
        this.filterColumns[this.columnNumber].actions.find(
          (a) => a.name == "alignl_" + this.columnNumber
        ).checked = false;
        this.filterColumns[this.columnNumber].actions.find(
      ...
      case "aedit": // Config Mode Only
        this.filterColumns[this.columnNumber].actions.find(
          (a) => a.name == "aedit_" + this.columnNumber
        ).checked ^= true; // Flip True-False Value
        this.columns = [...this.filterColumns];
      ...
      case "afilter": // Config Mode Only
        this.filterColumns[this.columnNumber].actions.find(
          (a) => a.name == "afilter_" + this.columnNumber
        ).checked ^= true; // Flip True-False Value
        this.columns = [...this.filterColumns];
      ...
      case "clipText":
        if (
          this.filterColumns[this.columnNumber].fieldName !=
            this.objectLinkField ||
          !this.not_suppressNameFieldLink
        ) {
          // Salesforce always forces Wrap Text on the Object's 'Name' field
      ...
      case "flex": // Config Mode Only
        if (
          typeof this.filterColumns[this.columnNumber].actions.find(
            (a) => a.name == "flex_" + this.columnNumber
          ).checked === "object"
        ) {
          this.filterColumns[this.columnNumber].actions.find(
            (a) => a.name == "flex_" + this.columnNumber
          ).checked = this.filterColumns[this.columnNumber].actions.find(
            (a) => a.name == "flex_" + this.columnNumber
          ).checked.flex;
        }
        this.filterColumns[this.columnNumber].actions.find(
      ...
      case "label": // Config Mode Only
        ...
        this.columnFilterValue = this.columnFilterValue
          ? this.columnFilterValue
          : this.baseLabel;
        this.columnType = "richtext";
        this.inputType = convertType(this.columnType);
        this.inputFormat =
          this.inputType == "number" ? convertFormat(this.columnType) : null;
      ...
      case "clear":
        this.filterColumns[this.columnNumber].label = this.isConfigMode
          ? this.cols[this.columnNumber].label
          : this.baseLabel;
  ...
  setWidth(sizes) {
    ...
    this.basicColumns.forEach((colDef) => {
      colFlexWidth = this.columns[colNum].actions?.find(
        (a) => a.name == "flex_" + colNum
      )?.checked
        ? 0
        : sizes[colNum] == 0 && this.isConfigMode
          ? DEFAULT_COL_WIDTH
          : sizes[colNum]; // v4.3.1 Reset column width when Flex is toggled off
  ...
  handleSearchChange(event) {
    ...
    if (
      this.searchTerm.length < MIN_SEARCH_TERM_SIZE &&
  ...
  handleSelectAllEdit() {
    ...
    this.filterColumns.forEach((colDef) => {
      colDef["actions"].find((a) => a.name == "aedit_" + colNum).checked =
        this.isAllEdit;
      colNum += 1;
    });
  ...
  handleSelectAllFilter(event) {
    ...
    let colNum = 0;
    this.filterColumns.forEach((colDef) => {
      colDef["actions"].find((a) => a.name == "afilter_" + colNum).checked =
        this.isAllFilter;
      colNum += 1;
    });
  ...
  handleSelectAllFlex() {
    ...
    let colNum = 0;
    this.filterColumns.forEach((colDef) => {
      colDef["actions"].find((a) => a.name == "flex_" + colNum).checked =
        this.isAllFlex;
      colNum += 1;
    });
  ...
  handleOpenFilterInput() {
    ...
    this.isFilterBlankValues =
      this.columnFilterValue == FILTER_BLANKS ? true : false;
  ...
  handleCloseFilterInput() {
    ...
    if (this.columnType == "boolean") {
      ...
      if (firstChar == "t" || firstChar == "y" || firstChar == "1") {
        // True, Yes, 1 - allow multiple ways to select a True value
  ...
  filterColumnData() {
          ...
          rows.forEach((row) => {
            ...
            for (let col = this.rowActionColumnOffset; col < colCount; col++) {
              ...
              if (fieldName.endsWith("_lookup")) {
                fieldName =
                  fieldName.slice(0, fieldName.lastIndexOf("_lookup")) +
                  "_name";
              }
              if (
                this.columnFilterValues[col] &&
                this.columnFilterValues[col] != null
              ) {
                if (
                  this.filterColumns[col].type != "boolean" &&
                  (!row[fieldName] || row[fieldName] == null)
                ) {
                  // The field is empty
                  ...
                  if (this.columnFilterValues[col] != FILTER_BLANKS) {
                    switch (this.filterColumns[col].type) {
                      ...
                      case "currency":
                      case "percent":
                        if (row[fieldName] != this.columnFilterValues[col]) {
                          // Check for exact match on numeric fields
                          match = false;
                          break;
                        }
                        break;
                      ...
                      case "time":
                        ...
                        if (dt != this.columnFilterValues[col]) {
                          // Check for date match on date & time fields
                          match = false;
                          break;
                        }
                        break;
                      default:
                        ...
                        let filterValue = this.columnFilterValues[col]?.replace(
                          /[.*+?^${}()|[\]\\]/g,
                          "\\$&"
                        ); // v4.3.5 escape special characters   ;
                        if (!this.matchCaseOnFilters) {
                          fieldValue = fieldValue.toLowerCase();
                          filterValue = filterValue.toLowerCase();
                        }
                        if (fieldValue.search(filterValue) == -1) {
                          // Check for filter value within field value
                          match = false;
                          break;
                        }
                    }
                ...
                this.filterColumns[col].actions.find(
                  (a) => a.name == "clear_" + actionColRef
                ).disabled = false;
              } else {
                if (
                  this.filterColumns[col].actions &&
                  this.filterColumns[col].actions != null
                ) {
                  // *** v4.2.1 fix ***
                  this.filterColumns[col].actions.find(
                    (a) => a.name == "clear_" + actionColRef
    ...
    this.showClearFilterButton =
      !this.hideClearSelectionButton &&
      !this.columnFilterValues.every((cfv) => cfv === null);
  }
          ...
          if (!this.isConfigMode) {
            ...
            rows.forEach((row) => {
              ...
              for (let col = 0; col < colCount; col++) {
                ...
                if (
                  cols[col].type != "boolean" &&
                  (!row[fieldName] || row[fieldName] == null)
                ) {
                  // No match because the field is boolean or empty or it's a row action
                  continue;
                }
                ...
                switch (cols[col].type) {
                  ...
                  default:
                    let fieldValue = row[fieldName].toString();
                    let filterValue = searchTerm?.replace(
                      /[.*+?^${}()|[\]\\]/g,
                      "\\$&"
                    ); // v4.3.5 escape special characters
                    if (!this.matchCaseOnFilters) {
  ...
  updateAlignmentParam() {
    ...
    this.filterColumns.forEach((colDef) => {
      let configAlign =
        convertType(colDef["type"]) != "number" ? "left" : "right";
      ...
      if (currentAlign && currentAlign != configAlign) {
        colString =
          ...
          colDef["fieldName"] +
          ":" +
          colDef["cellAttributes"]["alignment"];
      }
    });
    ...
    this.basicColumns.forEach((colDef) => {
      if (colDef["label"] != this.filterColumns[colNum].label) {
        colString =
          ...
          colDef["fieldName"] +
          ":" +
          this.filterColumns[colNum].label;
      }
      colNum += 1;
    });
    ...
    this.basicColumns.forEach((colDef) => {
      if (colDef["icon"] != this.filterColumns[colNum].iconName) {
        colString =
          ...
          colDef["fieldName"] +
          ":" +
          this.filterColumns[colNum].iconName;
      }
      colNum += 1;
    });
    ...
    this.basicColumns.forEach((colDef) => {
      if (colDef["wrapText"] != this.filterColumns[colNum].wrapText) {
        colString =
          ...
          colDef["fieldName"] +
          ":" +
          this.filterColumns[colNum].wrapText;
      }
      colNum += 1;
    });
  ...
  dispatchStringOutputs() {
    if (this.isUserDefinedObject) {
      ...
      this.outputRemainingRowsString = JSON.stringify(
        this._outputRemainingRows
      );
      ...
      this.dispatchEvent(
        new FlowAttributeChangeEvent(
          "outputEditedRowsString",
          this.outputEditedRowsString
        )
      );
      this.dispatchEvent(
        new FlowAttributeChangeEvent(
      ...
      this.dispatchEvent(
        new FlowAttributeChangeEvent(
          "outputRemovedRowsString",
          this.outputRemovedRowsString
        )
      );
      this.dispatchEvent(
        new FlowAttributeChangeEvent(
          "outputRemainingRowsString",
          this.outputRemainingRowsString
        )
      );
      this.dispatchEvent(
        new FlowAttributeChangeEvent(
          "outputActionedRowString",
          this.outputActionedRowString
        )
      );
    }
  ...
  validate() {
    ...
    this.outputSelectedRows.forEach((srow) => {
      const selData = this._tableData.find(
        (d) => d[this.keyField] == srow[this.keyField]
      );
      sdata.push(selData);
    });
    this.isUpdateTable = false;
    ...
    console.log(
      this.consoleLogPrefix + "outputSelectedRows",
      this.outputSelectedRows.length,
      SHOW_DEBUG_INFO ? this.outputSelectedRows : "***"
    );
    console.log(
      this.consoleLogPrefix + "outputEditedRows",
      this.outputEditedRows.length,
      SHOW_DEBUG_INFO ? this.outputEditedRows : "***"
    );
    console.log(
      this.consoleLogPrefix + "outputRemovedRows",
      this.outputRemovedRows.length,
      SHOW_DEBUG_INFO ? this.outputRemovedRows : "***"
    );
    console.log(
      this.consoleLogPrefix + "outputRemainingRows",
      this.outputRemainingRows.length,
      SHOW_DEBUG_INFO ? this._outputRemainingRows : "***"
    );
    console.log(
      this.consoleLogPrefix + "outputActionedRow",
      SHOW_DEBUG_INFO ? this._outputActionedRow : "***"
    );
